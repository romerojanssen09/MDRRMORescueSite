                ...
                <div>
                    ...
                    <select name="specialization_select" id="specialization_select" class="input-field text-sm @error('specialization') border-accent-400 @enderror" onchange="handleSpecializationChange()" required>
                        ...
                        @foreach($specializations as $spec)
                            <option value="{{ $spec->name }}" {{ old('specialization', $team->specialization) === $spec->name ? 'selected' : '' }}>
                                {{ $spec->name }}
                            </option>
                        @endforeach
                    ...
                    <input type="text" 
                           ...
                           value="{{ old('specialization', $team->specialization) }}" 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // If current specialization is not in dropdown, show as "Other"
    const currentSpec = input.value;
    let found = false;
    for (let option of select.options) {
        if (option.value === currentSpec) {
            found = true;
            break;
        }
    }
