<?php $__env->startSection('title', 'Emergency Reports'); ?>
<?php $__env->startSection('header', 'Emergency Reports'); ?>
<?php $__env->startSection('subtitle', 'Manage and assign emergency incidents'); ?>
...
<div class="card p-5 mb-6 border border-primary-700">
    <div class="flex items-center justify-between mb-4">
        ...
        <?php if(request()->hasAny(['status', 'emergency_type', 'search'])): ?>
    ...
    <form method="GET" id="filter-form" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
            ...
            <select name="emergency_type" class="input-field text-sm w-full" onchange="this.form.submit()">
                ...
                <option value="Fire" <?php echo e(request('emergency_type') === 'Fire' ? 'selected' : ''); ?>>Fire</option>
                <option value="Flood" <?php echo e(request('emergency_type') === 'Flood' ? 'selected' : ''); ?>>Flood</option>
                <option value="Earthquake" <?php echo e(request('emergency_type') === 'Earthquake' ? 'selected' : ''); ?>>Earthquake</option>
                <option value="Medical Emergency" <?php echo e(request('emergency_type') === 'Medical Emergency' ? 'selected' : ''); ?>>Medical Emergency</option>
                <option value="Accident" <?php echo e(request('emergency_type') === 'Accident' ? 'selected' : ''); ?>>Accident</option>
                <option value="Other" <?php echo e(request('emergency_type') === 'Other' ? 'selected' : ''); ?>>Other</option>
            </select>
        </div>
        ...
        <div>
            ...
            <div class="relative">
                <input type="text" name="search" id="search-input" placeholder="Search location..." class="input-field text-sm w-full pl-10" value="<?php echo e(request('search')); ?>">
    ...
    <div class="overflow-x-auto">
        <table class="w-full" id="reports-table">
            ...
            <tbody class="divide-y divide-primary-700">
                ...
                <tr data-report-id="<?php echo e($report->id); ?>" class="hover:bg-primary-700 transition">
                    ...
                    <td class="px-5 py-4">
                        <span class="text-sm text-primary-200">
                            ...
                            ?>
                            <?php echo e(Str::limit($formattedLocation, 40)); ?>

                        </span>
                    </td>
                    <td class="px-5 py-4">
                        <span class="text-sm text-primary-200"><?php echo e($report->citizen->full_name ?? 'N/A'); ?></span>
                    </td>
                    <td class="px-5 py-4">
                        <?php if($report->assignedTeam): ?>
                            <span class="inline-flex items-center text-sm text-secondary-400">
                                <i class="fas fa-users mr-1 text-xs"></i>
                                <?php echo e($report->assignedTeam->team_name); ?>

                            </span>
                        <?php elseif($report->assignedRescuer): ?>
                            <span class="inline-flex items-center text-sm text-secondary-400">
                                <i class="fas fa-user mr-1 text-xs"></i>
                                <?php echo e($report->assignedRescuer->full_name); ?>
                    ...
                    <td class="px-5 py-4">
                        <span class="status-badge" data-status="<?php echo e($report->status); ?>">
                    ...
                    <td class="px-5 py-4 whitespace-nowrap">
                        <div class="flex items-center space-x-2">
                            ...
                            <form action="<?php echo e(route('admin.reports.destroy', $report)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this report?')">
                                <?php echo csrf_field(); ?>
    ...
    <div class="p-5 border-t border-primary-700">
        <div class="flex items-center justify-between">
            <div class="text-sm text-primary-400">
                Showing <?php echo e($reports->firstItem()); ?> to <?php echo e($reports->lastItem()); ?> of <?php echo e($reports->total()); ?> results
            </div>
            <div class="flex space-x-2">
                ...
                <?php else: ?>
                    <a href="<?php echo e($reports->appends(request()->query())->previousPageUrl()); ?>" class="px-3 py-1.5 text-sm bg-primary-700 hover:bg-primary-600 text-primary-200 rounded transition">Previous</a>
                <?php endif; ?>

                <span class="px-3 py-1.5 text-sm bg-primary-700 text-primary-300">
                    Page <?php echo e($reports->currentPage()); ?> of <?php echo e($reports->lastPage()); ?>

                </span>

                <?php if($reports->hasMorePages()): ?>
                    <a href="<?php echo e($reports->appends(request()->query())->nextPageUrl()); ?>" class="px-3 py-1.5 text-sm bg-secondary-400 hover:bg-secondary-500 text-primary-950 rounded transition font-medium">Next</a>
                <?php else: ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\New folder (4)\MDRRMORescueApp\MDRRMOSite\resources\views\admin\reports\index.blade.php ENDPATH**/ ?>